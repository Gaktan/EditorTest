...
public class ShaderProgram {
	...
	public void attachFragmentShader(String name) {
		...
		if (GL20.glGetShaderi(fragmentShaderID, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
			System.err.println("Unable to create fragment shader \"" + name + "\" :");
	...
	public void attachVertexShader(String name) {
		...
		if (GL20.glGetShaderi(vertexShaderID, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
			System.err.println("Unable to create vertex shader \"" + name + "\" :");
	...
	public void setUniform(Uniform uniform, Matrix4 value) {
		int location = getLocation(uniform);
		if (checkLocation(location)) {
			GL20.glUniformMatrix4(location, false, value.toFloatBuffer());
		}
	}
	...
	public void setUniform(String name, float value1, float value2) {
		...
		if (checkLocation(location)) {
			GL20.glUniform2f(GL20.glGetUniformLocation(programID, name), value1, value2);
		}
	}
	...
	public void setUniform(Uniform uniform, Vector3 v) {
		int location = getLocation(uniform);
		if (checkLocation(location)) {
			GL20.glUniform3f(location, v.getX(), v.getY(), v.getZ());
		}
	}
	 ...
	 * @param f3
	 */
	public void setUniform(Uniform uniform, float f0, float f1, float f2, float f3) {
		int location = getLocation(uniform);
		if (checkLocation(location)) {
	...
	public int getLocation(Uniform u) {
		return uniforms[u.getValue()];
	}
